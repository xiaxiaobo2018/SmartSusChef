...
class PipelineConfig:
    ...
    tree_features: list[str] = field(
        default_factory=lambda: [
            ...
            "trend_ratio",
            "expanding_mean",
            "lag_same_weekday_avg",
            "lag_same_weekday_std",
        ]
    )
    tree_cat_features: list[str] = field(
        default_factory=lambda: ["day_of_week", "month", "is_public_holiday"]
    )
...
def add_lag_features(df):
    ...
    weekday_shifts = pd.concat([df["sales"].shift(s) for s in [7, 14, 21, 28]], axis=1)
...
def process_dish(dish_name, shared_df, country_code, config):
    ...
    ml = lgb.LGBMRegressor(
        n_estimators=1000, random_state=config.random_seed, n_jobs=-1, verbose=-1, **lgb_prod_params
    )
    ...
    return {
        ...
        "champion": champion,
        "mae": scores,
        "best_params": {"lightgbm": l_params, "catboost": c_params, "xgboost": x_params},
