...
def align_dates(df_target, df_ref, desc="data"):
    ...
    if abs(diff) > 1:
        print(
            f"[{desc}] Detected year mismatch (Ref: {ref_year}, Target: {target_year}). Shifting target by {diff} years."
        )
...
def load_data():
    ...
    if "quantity" not in df_food.columns or "dish_name" not in df_food.columns:
        raise ValueError(
            f"Missing required columns (quantity, dish_name). Found: {list(df_food.columns)}"
        )
    ...
    future_dates = pd.date_range(
        start=last_food_date + pd.Timedelta(days=1), periods=FORECAST_HORIZON
    )
    ...
    for col in weather_cols:
        if pd.api.types.is_numeric_dtype(df_weather[col]):
            ...
            df_weather_future[col] = (
                df_weather[col].mode()[0] if not df_weather[col].mode().empty else 0
            )
...
def prepare_holiday_data(df_holidays):
    ...
    # Find event column
    possible_events = [
        c for c in df_holidays.columns if "event" in c or "holiday" in c or "name" in c
    ]
    if possible_events:
...
def train_and_predict(dish_name, df_dish, df_weather_all, weather_cols, df_events):
    ...
    if not df_events.empty:
        ...
        events_df = pd.DataFrame({"event": "holiday_event", "ds": df_events["ds"]})
    ...
    future = m.make_future_dataframe(
        history_df, periods=FORECAST_HORIZON, n_historic_predictions=True
    )
...
def save_results(dish_name, forecast, model):
    ...
    final_df = final_df.rename(
        columns={"yhat1": "yhat", "yhat1 10.0%": "yhat_lower", "yhat1 90.0%": "yhat_upper"}
    )
    ...
    try:
        ...
        for dish in unique_dishes:
            try:
                ...
                forecast, model = train_and_predict(
                    dish, df_dish, df_weather_all, weather_cols, df_events
                )
