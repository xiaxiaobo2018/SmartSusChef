...
def main():
    try:
        ...
        sales_with_recipe = pd.merge(
            daily_sales, df_menu[["PLU", "RecipeId"]], on="PLU", how="inner"
        )
        ...
        final_df = pd.merge(
            final_daily,
            df_ingr_names[["IngredientId", "IngredientName"]],
            on="IngredientId",
            how="inner",
        )

        final_df = final_df.rename(
            columns={"date": "date", "IngredientName": "dish_name", "total_usage": "quantity"}
        )
